...
const Navbar = () => {
  ...
  <div className="flex-col md:flex-row flex items-center justify-between py-4">
      ...
      {navItems.map((item, index) => {
           return (
             <div className='flex items-center  md:ml-4 text-xs    space-x-2  py-2 pl-3  text-gray-700 rounded hover:bg-gray-200 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-500 md:dark:hover:text-dark dark:hover:bg-gray-900 dark:hover:text-white md:dark:hover:bg-transparent' key={index}>
             <span >{item.link}</span>
             {item.isDropDown && (
                                 <svg
